...
describe('Lexing', () => {
  ...
  test('lexes a cstring immediately followed by another token', () => {
    const text = '"hi"INTEGER';
    const tokens = Array.from(lex(text));
    assertEqual(tokens.length, 2);
    assertEqual(tokens[0].type, ProductionType.cstring);
    assertEqual(text.slice(tokens[0].location.startIndex, tokens[0].location.endIndex), '"hi"');
    assertEqual(tokens[1].type, ProductionType._INTEGER);
  });

  test('throws on a lone apostrophe instead of emitting an apostrophe token', () => {
    assert.throws(
      () => Array.from(lex("'")),
      (error) =>
        error instanceof ASN1SyntaxError &&
        error.message.includes('Unterminated single-quoted token'),
    );
  });
